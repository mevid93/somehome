...
public interface FriendRequestRepository extends JpaRepository<FriendRequest, Long> {
    ...
    @Query("SELECT a FROM Account a WHERE a IN (SELECT r.sender FROM FriendRequest r WHERE r.receiver =:account AND r.accepted = TRUE) OR a IN (SELECT r.receiver FROM FriendRequest r WHERE r.sender =:account AND r.accepted = TRUE) ORDER BY a.profilename ASC")
